...
exports.generate = async ({ size = DEFAULT_IMAGE_SIZE, seed } = {}) => {
    if (!seed) {
        // Generate random seed if one is not provided
    ...
    const lowerLimit = 3,
          upperLimit = 4; // Arbitrary values for determining square sizes
    ...
    while (x < imageSize && y < imageSize) {
        // Go until we are passed the image dimensions
...
function getBlockColor(colors, blockRow, blockCol) {
    ...
    if (blockCol % 2 !== 0 && blockRow % 2 !== 0) {
        ...
        return [Math.floor((colors[1][0] + colors[2][0]) / 2), Math.floor((colors[1][1] + colors[2][1]) / 2), Math.floor((colors[1][2] + colors[2][2]) / 2)];
    }
}
